...
{
	// 该函数 diag_test_aapn 的主要任务是快速评估两个序列（一个预处理好的“序列1”和一个待比较的“序列2”）之间是否存在相似区域。
	// 它利用序列中相邻氨基酸对（AAP）的信息，在比对矩阵中“打分”各个对角线（即比对中“j–i”的各个偏移量），
	// 从而寻找一个最佳的对角带（band）。这个过程作为一种快速过滤手段，
	// 在不进行完整动态规划比对之前就能判断序列对是否值得进一步比对，从而大大加速大规模序列聚类任务（CD‐HIT 的核心任务）。
	int i, i1, j, k;
	int* pp;
	// 总对角数
	...
	for (i = 0; i < len22; i++, i1++) {
		...
		// 如果该组2-mer 两个氨基酸向东，则cpx=1，否在fpx=2
		cpx = 1 + (iseq2[i] != iseq2[i + 1]);
		// 即该2-mer在新传入的序列2-mer中不存在
		if ((j = taap[c22]) == 0) continue;
		// m是c22的起始位置
		int m = aap_begin[c22];
		// i1 是新序列的长度-1
		// 此处的循环可以遍历该2-mer 在传入序列所有同样2-mer中
		for (int k = 0; k < j; k++) {
			// i1-aap_list[m+k] 是新序列和代表序列的矩阵中的对角线表示
			// diag_score +1表示发现在该对角线上又发现一个匹配的相似对，其脚标对应对角线编号
			// diag_score2 表示加权得分
	...
	// 向左扫描：如果窗口左侧某个对角线距离 imax_diag 超过 emax 或得分太低（diag_score[j] < 1），则剔除该部分。
	// 向右扫描：同理，从右侧向 imax_diag 靠拢，剔除得分低或距离过远的部分。
	// 这样保证最终确定的对角带两端只包含那些具有足够支持（匹配对数足够、距离中心合理）的对角线。
	...
	// 这样保证最终确定的对角带两端只包含那些具有足够支持（匹配对数足够、距离中心合理）的对角线。
	// 如果两条序列相似，匹配的AAP往往会沿着某个固定的偏移（即对角线）出现。
	// 通过寻找这一“对角带”，函数可以直观地捕捉到两序列的匹配趋势和局部对齐情况。
	...
	int band_left, int band_center, int band_right, WorkingBuffer& buffer)
{
	/*
	input:
		iseq1 是比对序列，iseq2 是代表序列，len1、len2为对应的序列长度
		mat 是对应的打分矩阵，open罚分为-11，extend罚分为-1
		band 是由之前函数diag找到的比对带
	output:
		best_score：最终比对得到的最高得分。
		iden_no：比对中相同的（identical）残基数。
		alnln：比对的长度（包含缺口）。
		dist：由不匹配位置数目与比对长度计算得到的距离（可用于后续评估序列间的相似性）。
		alninfo：如果提供，会填入比对起始和结束位置等详细信息。
	buffer:
		score_mat 打分矩阵
		back_mat 回溯矩阵
	*/
  ...
  = 11 5         \Xxxx\xxxxxxxxxxxx      \Xxxxxxx\xxxxxxxxx    xxxxxx\xxxxx\xxxx
	...
	for (i = 1; i <= len1; i++) {
		...
		for (j1 = J0; j1 <= J1; j1++) {
			...
			// 额外加上一个“奖励分”，这个奖励与当前单元格离带中部的距离有关，目的是鼓励比对结果尽量落在预期的对角线上，从而提高比对可靠性。
...
void WordTable::PrintAll()
{
	...
	for (i = 0;i < sequences.size();i++) {
		cout << sequences[i]->cluster_id << " " << sequences[i]->data << endl;
	}

	std::cout << "total sequences: " << sequences.size() << endl;
}
	...
	bool est, int min)
{
	// Summary:
	// 首先来看参数
	// aan_no 指新加入table的氨基酸序列产生的 k-mer 的个数,word_encodes 是k-mer有序的Hash(k-mer)集合，word_encodes_no 是其对应的
	// 集合差分计数数组
	// lookCounts 记录传入的序列与各代表序列相同的k-mer个数
	// indexMapping 表示 序号为index 的 序列，在lookCounts中对应的索引位置
	// lookCounts[indexMapping[index]-1].count 表示 该序列与 序号为 index 的序列相同的k-mer个数
	// lookCounts[indexMapping[index]-1].index 表示 该内容对应的index 序号。
	// 大体流程是：
	// 第一层循环遍历 buffer 中的k-mer集合
	// 第二层循环遍历该k-mer对应的Hash表
	// table 的更新在(即indexCounts)在ClusterOne中，在CheckOne系列函数中不涉及
	// special:
	// indexCounts 是一个二维数组，可以理解为 indexCounts[Hash(k-mer)][index] 表示代表序列index中Hash(k-mer)的数量
	...
	for (; j0 < aan_no; j0++, we++, wen++) {
		j = *we;
		j1 = *wen;
		// j 是具体的k-mer, j1 是该k-mer 的具体数量
		//if( j1 >1 ) printf( " %3i", j1 );
		if (j1 == 0) continue;

		// one 获取对应indexCount中对应为j的k-mer的Index组对，记录对应的序列号和数量
		NVector<IndexCount>& one = indexCounts[j];

		// k1 表示one里面index组对的总数
		k1 = one.Size();

		// 这里表示one 的具体数据，ic 指向 one 数组的数据指针
		...
		for (k = 0; k < k1; k++, ic++) {
			...
			// c 表示一个Index 键对 表示的在某个序列中的该k-mer的个数
			int c = ic->count < j1 ? ic->count : j1;

			// indexMapping 是一个大小为 sequences.size 的NVector数组，这样就可以把指针指定到对应的位置
			// 这样 *idm 表示indexMapping 中对应的 seq_no 的索引，即对应的 seq_no 在lookCounts 中的具体索引位置
			uint32_t* idm = indexMapping.items + ic->index;

			if (*idm == 0) {
				if (rest < min) continue;
				// 将lookCounts 的指针移动到末尾
...
void SequenceDB::MergeExternalSorting(const int first_size,const int upper_size ,const Options& options) {
	...
	node_chunks = (total_seqs - m) / (worker_size * upper_size) + 1;
	if ((total_seqs - m) % (worker_size * node_chunks))
		chunk_size = (total_seqs - m) / (worker_size * node_chunks) + 1;
	else chunk_size = (total_seqs - m) / (worker_size * node_chunks);

	// Modify: 将在DoClustering中的frag_no 的计算放入此处进行计算
	frag_nums = total_seqs;
	if (options.frag_size) frag_nums = 0;
	...
	while (!heap.empty()) {
		...
		if (options.frag_size) {
			frag_nums += (top->length - options.NAA) / options.frag_size + 1;
		}
		if (top->readNext()) {
			heap.push(top);
		}
...
bool SequenceDB::checkRepSeqs() {
	int n = rep_seqs.size();
	ofstream out("tempfiles/check_rep_seq.log");
	if(!out)
		throw runtime_error("Cannot open output file: tempfiles/check_rep_seq.log");
	for (int i = 0;i < n;i++) {
		int index = rep_seqs[i];
		
		out << "id " << index << " ";
		out << sequences[index]->state << " ";
		out << sequences[index]->cluster_id << " ";
		out << "\n";
	}
	out.close();
	return true;
...
void SequenceDB::SortDivide(Options& options, bool sort)
{
	...
	// 因此在这里使用桶排序的时间复杂度将远小于蛋白质序列的条数
	// 现有的在home / ssd / protein / nr.fa 有289341353条序列
	// 因此在此处用桶排序的时间和空间原小于扫描一遍序列？ 
	if (sort) {
...
{
	// 将相邻的两个氨基酸分成一组，类似一个2-mer
	// aap_begin 表示每个 Hash后的2-mer 的数量，即aap_list 的起始索引
	// aap_list 表示一个2-mer在原始序列中第一次、第二次、... 、第taap[Hash(2-mer)]次的位置。
	// 假如我有一个2-mer，那么其第一次出现在原序列中的位置为:
	// aap_list[aap_begin[Hash(2-mer)]]
	// 该2-mer 在序列中出现的次数为 taap[Hash(2-mer)]
...
void SequenceDB::DoClustering_MPI(const Options& options, int my_rank, bool master, bool worker, int worker_rank) {
	...
	int seq_no = total_seqs;
	int frag_no = frag_nums;
	int frag_size = options.frag_size;
	int len, len_bound;
	int flag;

	// Modify：迁移至MergeExternalSorting中进行计算
	// if (frag_size) {
	// 	frag_no = 0;
	// 	if (master) {
	// 		for (i = 0; i < seq_no; i++)
		...
		for (i = 0;i < chunks.size();i++) {
			...
			if(remaining){
				...
				for (j = chunks[i].first;j < chunks[i].second;j++) {
					if(i>0&&i<chunks.size()-1&&j%100==0){
			...
			#pragma omp parallel for num_threads(T)
				for (int j = chunks[i + 1].first;j < chunks[i + 1].second;j++) {
					int index = (j - chunks[i + 1].first) * 7;
					Sequence* seq = sequences[j];
			...
			#pragma omp parallel for num_threads(T) schedule(dynamic, 1)
				...
				if (chunks_id[start] == soure_chunk + 1&&!remaining) {
					...
					for (j = chunks[start].first;j < chunks[start].second;j++) {
						int index = (j - chunks[start].first)*7;
						Sequence* seq = sequences[j];
...
int SequenceDB::CheckOneAA(Sequence* seq, WordTable& table, WorkingParam& param, WorkingBuffer& buf, const Options& options, bool test_flag)
{
	...
	IndexCount* ic = lookCounts.items;
	ic = lookCounts.items;
	// 此处由于lookCounts 为连续存储，这里ic->count != 0 都保存在前面，可由indexMapping查找一个序列具体对应的位置
	// 也可以用ic->index 找到对应序列的index
	for (; ic->count; ic++) {
		if (!frag_size) {
			// 随时清空
		...
		// seqi 表示当前序列的data，seqj表示遍历的代表序列的data
		...
		int rc = FAILED_FUNC;
		// std::cout << "param.aln_cover_flag: " << param.aln_cover_flag << "\n";
		// std::cout << "options.print: " << options.print << "\n";
		// std::cout << best_sum << "\n";
		// exit(0);
		//sw
		...
		/*
		根据比对得到的匹配比例（tiden_pc，即匹配残基数除以有效比对长度）或计算得到的距离（distance），
		再与预设的阈值（例如 cluster_thd 或 distance_thd）比较，判断当前候选代表与待检序列是否足够相似。
		*/
		...
		// std::cout << "options.has2D: " << options.has2D << "\n";
		// std::cout << "options.cluster_best: " << options.cluster_best << "\n";
		// exit(0);
...
{
	int i;
	// -NAA 应该为滑动窗口氨基酸数量,由 -n 参数设置
	// 下面若干cutoff均为聚类过程中的一系列阈值
	int NAA = options.NAA;
	double aa1_cutoff = options.cluster_thd;
	double aas_cutoff = 1 - (1 - options.cluster_thd) * 4;
	double aan_cutoff = 1 - (1 - options.cluster_thd) * options.NAA;
	...
	int len, len_bound;
	int flag;
	// std::cout << std::endl << ">>> seq_no :" << seq_no << std::endl;
	// std::cout << ">>> frag_no :" << frag_no << std::endl;
	...
	if (not options.isEST) {
		// 该函数计算并更新 aa1_cutoff、aa2_cutoff 和 aan_cutoff 三个阈值。
		cal_aax_cutoff(aa1_cutoff, aas_cutoff, aan_cutoff, options.cluster_thd,
			options.tolerance, naa_stat_start_percent, naa_stat, NAA);
	}
	...
	// For WorkingBuffer的过程
	// 通过多数据规模的测试可以发现该函数的运行时间大概在10ms左右，
	// 1M行的数据集大概为3ms，10M行大概6ms，100M行大概11ms，而/home/ssd/protein/nr.fa中大概2B行，用时大概
	// 但是在测试过程中发现前置的排序时间非常长，但是这里已经采用了桶排序，是否考虑优化方法？
	...
	// 这段代码主要是在多线程环境下，根据 max_memory 和 threads 计算哈希表的存储限制，以避免过度占用内存。
	// 所以说我们直接开多个线程每个线程单独执行会不会出现内存爆掉的问题？
	// 从代码里看如果需要更精准的的控制需要规定max_memory，里面有一个和scale参数和cdhit输入的-T 有关
	opts.ComputeTableLimits(min_len, max_len, len_n50, mem_need);
